...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if type(insect)==Ant or insect.name != "Queen" or insect.Queen == False:
                if self.ant is insect:
                    ...
                    if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
                        # print(self.ant.ant.name)
                        # print(self.ant.ant.Queen)
                        self.ant.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        # BEGIN Problem 3B
        if self.max_range != None: # this is the short ThrowerAnt
            count = 0
            beePos=self.place
            while not beePos.bees and beePos.entrance != hive and count < self.max_range:
                beePos=beePos.entrance
                count+=1
            return random_or_none(beePos.bees)
        elif self.min_range != None: #this is the Long ThrowerAnt
            beePos=self.place
            for _ in range(self.min_range):
                if beePos == hive:
                    return None
                beePos=beePos.entrance
            while not beePos.bees and beePos.entrance != hive:
                beePos=beePos.entrance
            return random_or_none(beePos.bees)
        else: # just incase if the function calls ThrowerAnt
            beePos=self.place
            while not beePos.bees and beePos.entrance != hive:
                beePos=beePos.entrance
            return random_or_none(beePos.bees)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.Queen:
            ...
            curPlace = self.place
            while curPlace.exit != None:
                curPlace = curPlace.exit
                if curPlace.ant  != None:

                    if checkDup(curPlace.ant,self.inspiredAnt) == False:
                        self.inspiredAnt.append(curPlace.ant)
                        curPlace.ant.damage *= 2
                    if isinstance(curPlace.ant,BodyguardAnt) and curPlace.ant.ant!=None:
                        if checkDup(curPlace.ant.ant,self.inspiredAnt)==False:
                            self.inspiredAnt.append(curPlace.ant.ant)
                            curPlace.ant.ant.damage *= 2
