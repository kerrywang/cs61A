...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        def map_pair(func,expr,env):
            if expr == nil:
                return nil
            return Pair(func(expr.first,env), map_pair(func,expr.second,env))
        if check_procedure(scheme_eval(first,env)) != SchemeError:
            return scheme_apply(scheme_eval(first,env),map_pair(scheme_eval,rest,env),env)
...
def eval_all(expressions, env):
    ...
    return scheme_eval(expressions.first,env,True) #change
    # END PROBLEM 8
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("incorrect argument")
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        if len(expressions.second) == 1:
            ...
            env.define(expressions.first,scheme_eval(expressions.second,env))
...
def do_if_form(expressions, env):
    ...
    if scheme_truep(scheme_eval(expressions.first, env)):
        return scheme_eval(expressions.second.first, env,True) #change
    elif len(expressions) == 3:
        return scheme_eval(expressions.second.second.first, env,True) #change
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    if expressions.second == nil and scheme_eval(expressions.first,env,True):
        return scheme_eval(expressions.first,env,True)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        key = Pair(bindings.first.first, key)
        value = Pair(scheme_eval(bindings.first.second.first,env), value)
        bindings = bindings.second
    check_formals(key)

    child = env.make_child_frame(key,value)
